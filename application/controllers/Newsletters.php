		...
		if( $user['role'] == '99' || $user['role'] == '98' || $user['role'] == '95' || $user['role'] == '96'){
			...
			if( !empty($list) ){
				foreach ($list as $newsletter) {
					...
					$row[] = "<div class='d-flex'><a title='view' href=" . site_url("newsletters/view/{$newsletter->id}/{$newsletter->temp_key}") . " class='btn btn-success' ><i class='fa fa-eye'></i></a>" . $rowedit . $row1 . "</div>";
