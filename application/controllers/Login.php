...
class Login extends CI_Controller {
	...
	*/
		...
		if( $get_user ){
			...
			if( $count_res == 1 ){
				...
				if($user_status == "active"){
					//check if user login in today
					$this->__check_agents__standard_login_time( $user_info );
	...
	private function __check_agents__standard_login_time( $row ){
		...
		$login_request_date = date("Y-m-d", strtotime($row->login_request_date));
		$get_user_role 		= $row->user_type;
		$curr_date 			= date("Y-m-d");
		$last_login 		= !empty($row->last_login) ? date( "Y-m-d", strtotime($row->last_login) ) : "";
		
		//check if user role is 96 = sales team,
		if( in_array( $get_user_role, standard_login_roles() ) && is_standard_login() ){
			
			$current_time 		= date("h:i A");
			$stan_time = !empty( standard_login_time() ) ? explode( "-", standard_login_time() ) : "";
			$login_start_time 	= isset($stan_time[0]) && !empty( $stan_time[0] ) ? $stan_time[0] :"09:30 AM";
			$login_over_time 	= isset($stan_time[1]) && !empty( $stan_time[1] ) ? $stan_time[1] :"09:45 AM";
			
			//$login_start_time = "11:38 AM ";
			//$login_over_time 	= "11:50 AM";
			
			$current_time_f 	= DateTime::createFromFormat('H:i A', trim($current_time));
			$login_start_time_f = DateTime::createFromFormat('H:i A', trim($login_start_time));
			$login_over_time_f	= DateTime::createFromFormat('H:i A', trim($login_over_time));
			
			if( ( $current_time_f >= $login_start_time_f && $current_time_f <= $login_over_time_f ) || $curr_date == $last_login || ( $login_request == 0 &&  $login_request_date == $curr_date ) ){
				//condition here
				$error = "";
			}else if( $current_time_f < $login_start_time_f  ){
				$error = "You can't login. Your standard login time is ( " . $login_start_time . " - " .  $login_over_time . " )";
				$res = array('status' => false, 'msg' => $error );
				die( json_encode( $res ) );
			}else{
				//request only if try to login first time in a day
				if( $login_request == 1 &&  $login_request_date == $curr_date  ){
					$error = "Login request already sent to manager please contact your manager.";
					$res = array('status' => false, 'msg' => $error );
				}else{
					//request manager to login
					$loginData = array(
						'login_request_date' 	=> date('Y-m-d h:i:s'),
						'login_request' 		=>  1,
					);
					
					$this->global_model->update_data('users', array('user_id' => $row->user_id ), $loginData );
					
					$error = "Please contact your manager to login because your standard login time is ( " . $login_start_time . " - " .  $login_over_time . " )";
					
					$this->session->set_flashdata('error_msg',"Please contact your manager to login because your standard login time is over. Your standard login time ( " . $login_start_time . " - " .  $login_over_time . " )" );
