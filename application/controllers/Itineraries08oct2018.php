...
class Itineraries extends CI_Controller {
	...
	public function updateItiStatus(){
		...
		if( isset( $_POST['iti_id'] ) && !empty( $_POST['iti_id'] ) && isset( $_POST['callType'] ) ){
			...
			$doc_path =  dirname($_SERVER["SCRIPT_FILENAME"]).'/site/assets/client_docs/';
			//Get customer info
			...
			if( $callType == "Picked call" ){
				...
				$this->global_model->update_data( "customers_inquery", array("customer_id" => $customer_id ) , array("decline_comment" => $decline_comment , "cus_status" => 8 ) );
				...
				if( isset( $_FILES['client_aadhar_card']['name'] ) && !empty( $_FILES['client_aadhar_card']['name'] ) ){
					...
					if(!$this->upload->do_upload('client_aadhar_card')){
						...
						$aahar_card_image_name = $data['file_name'];
					}
				}
				...
				if( isset( $_FILES['payment_screenshot']['name'] ) && !empty( $_FILES['payment_screenshot']['name'] ) ){
					...
					$n = str_replace(' ', '_', $f_n);
					$file_name = $iti_id . "_payment_{$u_id}_"  . $n;
					...
					if(!$this->upload->do_upload('payment_screenshot')){
						...
						$data = $this->upload->data();
						$payment_image_name = $data['file_name'];
					}
				}
			...
			*/
			...
			}else if( !empty( $nxt_call ) && $callType != "Close lead" ){
				$title 		= "Iti Followp";
			...
			if( $action == "approved" ){
				if( isset( $_FILES['client_aadhar_card']['name'] ) && !empty( $_FILES['client_aadhar_card']['name'] ) ){
					...
					if(!$this->upload->do_upload('client_aadhar_card')){
						...
						$data = $this->upload->data();
						$update_data['client_aadhar_card'] = $data['file_name'];
					}
				}
				...
				if( isset( $_FILES['payment_screenshot']['name'] ) && !empty( $_FILES['payment_screenshot']['name'] ) ){
					...
					if(!$this->upload->do_upload('payment_screenshot')){
						...
						$data = $this->upload->data();
						$update_data['payment_image_name'] = $data['file_name'];
					}
				}
		...
		//if( $reject_type == 2 ){
			...
			if( isset( $_FILES['client_aadhar_card']['name'] ) && !empty( $_FILES['client_aadhar_card']['name'] ) ){
				...
				if(!$this->upload->do_upload('client_aadhar_card')){
					...
					$data = $this->upload->data();
					$payData["client_aadhar_card"] = $data['file_name'];
				}
			}
			...
			if( isset( $_FILES['payment_screenshot']['name'] ) && !empty( $_FILES['payment_screenshot']['name'] ) ){
				...
				if(!$this->upload->do_upload('payment_screenshot')){
					...
					$data = $this->upload->data();
					$payData["payment_screenshot"] = $data['file_name'];
				}
			}
