	...
	<div class="page-content-wrapper">
		...
		?>
			<?php if( !empty($itinerary[0] ) ){ 			
												...
												<tbody>
													...
													foreach( $iti_clients_docs as $ind => $doc ){
														echo "<tr>";
															...
															echo "<td>"; ?>
																<a href="<?php echo $doc_path . $doc->file_url; ?>" target="_blank" class="btn_eye" style="position:relative;">
									...
									<div class="portlet-body ">
										...
										<table class="table table-bordered">
											...
											<tbody>
												...
												<?php }else{ ?>
													<tr>
														<td colspan="4">No Volvo/Train/Flight booking create against this itinerary.</td>
													</tr>	
												<?php } ?>
											</tbody>
										</table>
												...
												if( $hotel_bookings ){ 
													...
													foreach( $hotel_bookings as $h_book ){
														...
														<tr>
															...
															<td><?php echo $status; ?></td>
															<td><a title='View' href="<?php echo site_url("hotelbooking/view/{$h_book->id}/{$h_book->iti_id}"); ?>" class='btn_eye' ><i class='fa fa-eye' aria-hidden='true'></i></a></td>
														</tr>
													<?php 
														$ch++;
													} ?>
				...
				<?php /* if( isset( $paymentDetails[0] ) && !empty( $paymentDetails[0] ) && $paymentDetails[0]->approved_by_account_team == 0 ){ ?>
					<hr>
						<form id="generate_invoice_frm">
							<h3 class='text-center'> GENERATE BANK RECEIPT AND PROCEED ITINERARY TO SERVICE TEAM</h3>
							<div class="form-group">
								<textarea required placeholder="Please Enter comment here...." rows="4" cols="20" name="comment" class="form-control comment"/></textarea> 
							</div>
							<input type="hidden" name="iti_id" value="<?php echo $iti->iti_id; ?>">
							<input type="hidden" name="agent_id" value="<?php echo $iti->agent_id; ?>">
							<input type="hidden" name="temp_key" value="<?php echo $iti->temp_key; ?>">
							<input type="hidden" name="id" value="<?php echo $paymentDetails[0]->id; ?>">
							<input type="hidden" name="customer_id" id="customer_id" value="<?php echo $iti->customer_id; ?>">
							<div class="form-group col-md-12 row">
								<button id="LinkButton1" type="submit" class="btn green uppercase app_iti">GENERATE</button>
							</div>
							<div class="clearfix"></div>
