		...
		if( $count_day > 0 ){
			...
			for ( $i = 0; $i < $count_day; $i++ ) {
				...
				if( isset($tourData[$i]['hot_des'] ) && !empty( $tourData[$i]['hot_des'] ) ){
					...
					$hot_destination = '<div class="hot_destination_view"><div class="day_yellow">Hot Destination :</div>' . $hot_dest . '</div>';
				}	
				$pdf_html .= 
				<<<EOD
...
EOD;
   ...
   /*$pdf->Output( $pdf_name , 'I'); // the second option D forces the browser to download the PDF. Passing I will tell the browser to show it */
	$temp_path = "site/itinerary_pdf/";
	$time = time();
	$path = set_realpath($temp_path, TRUE);       // Shows an error, as the path cannot be resolved
	$file_path = $path . $pdf_name . "_" . $time . ".pdf";
