          ...
          <div class="col-md-7 right-wrapper">
                ...
                <ul class="nav nav-tabs" id="tab_lising" role="tablist">
                  ...
                  <div class="tab-content client-view">
					...
					<?php $hotel_meta = !empty($iti->hotel_meta) ? unserialize($iti->hotel_meta) : ""; 
					$standard_html = "";
					$deluxe_html = "";
					$super_deluxe_html = "";
					$luxury_html = "";
					...
					*/				
						...
						//Rate meta
						$rate_meta = !empty( $iti->rates_meta ) ? unserialize($iti->rates_meta) : "";
						...
						if( !empty( $rate_meta ) ){
							...
							$s_rates = isset( $rate_meta["standard_rates"] ) && !empty( $rate_meta["standard_rates"]) ? $rate_meta["standard_rates"] + $rate_meta["standard_rates"] * $agent_price_percentage / 100 : 0;
							$d_rates = isset( $rate_meta["deluxe_rates"] ) && !empty( $rate_meta["deluxe_rates"]) ? $rate_meta["deluxe_rates"] + $rate_meta["deluxe_rates"] * $agent_price_percentage / 100 : 0;
							$sd_rates = isset( $rate_meta["super_deluxe_rates"] ) && !empty( $rate_meta["super_deluxe_rates"]) ? $rate_meta["super_deluxe_rates"] + $rate_meta["super_deluxe_rates"] * $agent_price_percentage / 100: 0;
							$l_rates = isset( $rate_meta["luxury_rates"] ) && !empty( $rate_meta["luxury_rates"]) ? $rate_meta["luxury_rates"] + $rate_meta["luxury_rates"] * $agent_price_percentage / 100 : 0;
							
							$standard_rates = !empty( $s_rates ) ? number_format($s_rates) . "/- {$inc_gst} <br> {$s_pp} <br> {$child_s_pp}" : "--";
							$deluxe_rates = !empty( $d_rates ) ? number_format($d_rates) . "/- {$inc_gst} <br> {$d_pp} <br> {$child_d_pp}" : "--";
							$super_deluxe_rates = !empty( $sd_rates ) ? number_format($sd_rates) . "/- {$inc_gst} <br> {$sd_pp} <br> {$child_sd_pp}" : "--";
							$rate_luxry = !empty( $l_rates ) ? number_format($l_rates) . "/- {$inc_gst} <br> {$l_pp} <br> {$child_l_pp}" : "--";
							
							
							//Check for discountPriceData
							if( !empty( $discountPriceData ) ){
								foreach( $discountPriceData as $price ){
									...
									if( $sent_status ){	
										...
										$strike_class_dis = ($price !== end($discountPriceData) && $count_price > 1 ) ? "strikeLine" : "";
										
										
										$s_price_dis .= "<tr class='$strike_class_dis'><td>Price</td><td><strong>" . $s_price . "</strong></td>";
										$d_price_dis .= "<tr class='$strike_class_dis'><td>Price</td><td><strong>" . $d_price . "</strong></td>";
										$sd_price_dis .= "<tr class='$strike_class_dis'><td>Price</td><td colspan='2'><strong>" . $sd_price . "</strong></td>";
										$l_price_dis .= "<tr class='$strike_class_dis'><td>Price</td><td colspan='2'><strong>" . $l_price . "</strong></td>";
										
									}	
									
									/* echo "<tr class='$strike_class_dis'><td>Price</td><td><strong>" . $s_price . "</strong></td>";
									echo "<td><strong>" . $d_price . "</strong></td>";
									echo "<td><strong>" . $sd_price . "</strong></td>";
									echo "<td><strong>" . $l_price . "</strong></td>";
									echo "<td><strong>" . $total_cost_after_dis . "</strong></td></tr>"; */
								}
							}	
						}else{
							$standard_rates = "Coming Soon";
							$deluxe_rates = "Coming Soon";
							$super_deluxe_rates = "Coming Soon";
							$rate_luxry = "Coming Soon";
						} 
					...
					<hr>
					<div class="well well-sm"><h3><?php echo !empty( $iti->final_amount ) ? "Final Cost: <strong class='green'> " . number_format($iti->final_amount) . " /-</strong>" : "Rates"; ?></h3></div>
					<!-- Rate Meta -->
					...
					if( !empty( $rate_meta ) ){
						...
						//get per person price
						$per_person_ratemeta 	= unserialize($iti->per_person_ratemeta);
						$inc_gst = isset( $per_person_ratemeta["inc_gst"] ) && $per_person_ratemeta["inc_gst"] == 1 ? "(Inc. GST)" : "(Exc. GST)";
						$s_pp = isset( $per_person_ratemeta["standard_rates"] ) && !empty($per_person_ratemeta["standard_rates"]) ? "RS. " . number_format($per_person_ratemeta["standard_rates"] + $per_person_ratemeta["standard_rates"] * $agent_price_percentage/100) . "/- Per Person" : "";
						$d_pp = isset( $per_person_ratemeta["deluxe_rates"] ) && !empty($per_person_ratemeta["deluxe_rates"]) ? "RS. " . number_format($per_person_ratemeta["deluxe_rates"]  + $per_person_ratemeta["deluxe_rates"] * $agent_price_percentage/100) . "/- Per Person" : "";
						$sd_pp = isset( $per_person_ratemeta["super_deluxe_rates"] ) && !empty($per_person_ratemeta["super_deluxe_rates"]) ? "RS. " . number_format($per_person_ratemeta["super_deluxe_rates"]  + $per_person_ratemeta["super_deluxe_rates"] * $agent_price_percentage/100) . "/- Per Person" : "";
						$l_pp = isset( $per_person_ratemeta["luxury_rates"] ) && !empty($per_person_ratemeta["luxury_rates"]) ? "RS. " . number_format($per_person_ratemeta["luxury_rates"]  + $per_person_ratemeta["luxury_rates"] * $agent_price_percentage/100) . "/- Per Person" : "";
						...
						echo "<tr><td><strong class='red'>Price</strong></td>
							...
							<ul>
							<?php $hotel_note_meta = !empty( $iti->hotel_note_meta ) ? unserialize($iti->hotel_note_meta) : ""; 
                    ...
                    <div  id="incExcSection">
						...
						<div class="table-responsive inclusion">
						 <h5> Inclusion  </h5>
									<?php 
									$inclusion = !empty($iti->inc_meta) ? unserialize($iti->inc_meta) : ""; 
									$count_inc = count( $inclusion );
									$exclusion = !empty($iti->exc_meta) ? unserialize($iti->exc_meta) : ""; 
									...
									$sp_inc = !empty($iti->special_inc_meta) ? unserialize($iti->special_inc_meta) : ""; 
					...
					<div class="cart">
						<div class="row">
							<div class="text-right">
								...
								<?php }else{ ?>
									<a class="btn" href="https://www.Trackitinerary.com/welcome/hdfc" target="_blank" title="Book Now">Pay Now</a>
								<?php } ?>
							</div>
						</div>
					</div>
