					...
					<?php if($hotel_booking){ ?>
						...
						<?php if( $status == 9 ){ ?>
							<h3 class="approve_heading">Approve</h3>
								<?php echo "<strong>Booking Date : </strong> " . $hotel_booking->check_in . " To " . $hotel_booking->check_out; ?>
						...
						<?php } ?>
								...
								<input type="hidden" value="<?php echo $hotel_booking->hotel_id;?>" name="hotel_id" id="hotel_id">
	...
	$("#confirmForm").validate({
		submitHandler: function(form) {
			...
			if (ajaxReq) {
					...
					ajaxReq = 	$.ajax({
				type: "POST",
				url: "<?php echo base_url('confirm/update_hotel_booking_status'); ?>" ,
				dataType: 'json',
				data: formData,
				beforeSend: function(){
