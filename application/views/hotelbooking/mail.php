<link href="https://fonts.googleapis.com/css?family=Roboto:400,700" rel="stylesheet">
	...
	$mail_html .= "<div class='mail-logo'><img src='{$logo_url }'></div>";
			...
			//$without_extra_bed_cost = !empty($hotel_book->without_extra_bed_cost) ? $hotel_book->without_extra_bed_cost : 0;
			...
			$inclusion_cost = !empty($hotel_book->inclusion_cost) ? $hotel_book->inclusion_cost : 0;
			$hotel_tax 	= !empty($hotel_book->hotel_tax) ? $hotel_book->hotel_tax : 0;
			...
			$mail_html .= "<p style='font-weight: bold; font-size: 14px; text-align: left'>Kindly confirm the below booking <span style='font-weight:bold;'>:-</span> </p></div>"; 
			...
			//without extra bed cost per nights
			if( !empty( $without_extra_bed_cost ) ){
				$mail_html .= "<strong>Without Extra Cost: </strong><div class='info'>{$hotel_book->without_extra_bed_cost} * {$w_extra_bed} = {$without_extra_bed_cost} /-Per Night</div>"; 
			}
			...
			//Button section
			if( isset( $show_approved_btn ) && $show_approved_btn ){
				$mail_html .= "<div class='response_btn'><a title='Click to Approve Hotel Booking' href=" . site_url("confirm/hotelbooking?hotel_id=". base64_url_encode($hotel_book->hotel_id) . "&iti_id=" . base64_url_encode($hotel_book->iti_id) . "&token_key=" . base64_url_encode($hotel_book->id) . "&status=" . base64_url_encode(9)) . " class='btn btn-success' >Confirm</a>";
				$mail_html .=  "<a title='Click to Decline' href=" . site_url("confirm/hotelbooking?hotel_id=". base64_url_encode($hotel_book->hotel_id) . "&iti_id=" . base64_url_encode($hotel_book->iti_id) . "&token_key=" . base64_url_encode($hotel_book->id)  . "&status=" . base64_url_encode(8)) . " class='btn btn-danger' >Decline</a></div></div>";
			}
			
			//Approved or declined note
			if( isset( $booking_note ) && $booking_note ){
				$mail_html .= "<strong>Booking Note: </strong>{$booking_note}";
			}
