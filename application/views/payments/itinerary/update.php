			...
			<?php if( !empty($payment ) ){ 
				...
				<div class="portlet box blue">
					<div class="portlet-title">
						...
						{ Package Type: <strong class="red"> <?php echo check_iti_type( $pay->iti_id ); ?></strong> }
						</div>
						<a class="btn btn-success pull-right" href="<?php echo site_url("payments"); ?>" title="Back">Back</a>
					</div>
				</div>
				<div class="row">
				<div class="col-md-12">
				<div class="text-center">
					<a title='View Quotation' target="_blank" href=" <?php echo site_url("itineraries/view/{$pay->iti_id}/{$temp_key}") ; ?> " class='btn btn-danger' ><i class='fa fa-eye' aria-hidden='true'></i> View Quotation </a>
				</div>
					...
					<?php if( $refund_exist ){ ?>
						...
						<form id="refund_payment">
							...
							<p class="text-center"><strong><strong>Note By Agent : </strong> <span class='red'><?php echo $pay->amendment_note; ?></span></p>
							<div class="form-group col-md-3">
								<label for="usr">Total Refund Amount:</label>
								<input type="text"  readonly class="form-control" id="refund_amount" name="refund_amount" value="<?php echo $pay->refund_amount; ?>" />
							</div>
							
							<div class="form-group col-md-3">
								<label for="usr">Refund Due Date:</label>
								<input type="text"  readonly class="form-control" value="<?php echo $pay->refund_due_date; ?>" />
							</div>
							...
							<div class="form-group col-md-3">
								<div class="form-group2">
									...
									<input required class="form-control" id="pay_type" type="text" placeholder="eg: Cash/cheque/online" name="pay_type" value="">
								</div>	 
							</div>
							
							<div class="form-group col-md-2">
								<label for="usr">&nbsp;</label>
								<button type="submit" class="btn green uppercase submit_frm" id="submit_frm">Submit To Refund</button>
							</div>
					...
					<?php if( $pay_recieved >= 50  && $pay->iti_close_status == 0 && !$refund_exist ){ ?>
						<div class="close_iti text-right">
							<a href="javascript: void(0)" id="update_closeStatus" data-customer_id = "<?php echo $pay->customer_id; ?>" data-iti_id ="<?php echo $pay->iti_id; ?>" class="btn btn-success" title="click to close itinerary"><i class="fa fa-close" aria-hidden="true"></i> Close Itinerary</a>	
						</div>
						<hr>
					<?php }elseif( $pay->iti_close_status == 1 ){
						...
						<div class="close_iti text-right">
							<a href="javascript: void(0)" id="update_closeStatus" data-customer_id = "<?php echo $pay->customer_id; ?>" data-iti_id ="<?php echo $pay->iti_id; ?>" class="btn btn-success" title="click to close itinerary"><i class="fa fa-close" aria-hidden="true"></i> Close Itinerary (already closed)</a>	
						</div>
						...
						<div class="customer-details">	
							<div class=" ">
								...
								<div class="col-md-6 col-lg-6">
									...
									<div class="col-md-8 form_vr"><?php echo display_month_name( $pay->next_payment_due_date ); ?></div>
								</div>
								<div class="col-md-6 col-lg-6">
								...
								<div class="col-md-6 col-lg-6">
									...
									<div class="col-md-8 form_vr"><strong class='green'><?php echo display_month_name($pay->travel_date); ?></strong></div>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="col-md-12">
					<table class="table table-bordered d-table">
							<thead class="thead-default">
								<tr> <p class="text-center uppercase green"><strong>All Receipts</strong></p> </tr>
								<tr>
									<th> # </th>
									<th> Receipt Type </th>
									<th> Transfer Ref.</th>
								...
								if( !empty( $receipts ) ){ 
									...
									foreach( $receipts as $receipt ){ ?>
										<tr>
											...
											<?php echo "<td><a href=" . site_url("accounts/update_receipt/{$receipt->id}") . " class='btn btn-success ajax_edit_hotel_table' title='Update receipt' ><i class='fa fa-pencil'></i></a>
												<a href=" . site_url("accounts/view_receipt/{$receipt->id}") . " class='btn btn-success' title='view' ><i class='fa fa-eye'></i></a></td>"; ?>
										<tr>	
										<?php 
										$indx++;
									} 
				...
				<div class="col-md-12">
					<table class="table table-bordered d-table">
							<thead class="thead-default">
								<tr> <p class="text-center uppercase green"><strong>Payment Transactions</strong></p> </tr>
								<tr>
									<th> Sr.No</th>
									<th> Pay. Received</th>
									<th> Pay. Refund</th>
							...
							<tbody>
								<tr>
									...
									<td>0</td>
									<td><?php echo !empty($pay->booking_date) ? date("d M, Y",strtotime($pay->booking_date)) : ""; ?></td>
								...
								if( !empty( $payment_transaction ) ){ 
									foreach( $payment_transaction as $pay_trans ){ ?>
										<tr data-tra_id = "<?php echo $pay_trans->tra_id;?>">
											...
											<td class="e_bank_name" data-bank_name="<?php echo $pay_trans->bank_name; ?>"><?php echo $pay_trans->bank_name; ?></td>
											<td class="e_payment_type" data-payment_type="<?php echo $pay_trans->payment_type; ?>"><?php echo $pay_trans->payment_type; ?></td>
											<td><?php echo get_user_name($pay_trans->agent_id); ?></td>
											<td>
												<a title="Update details" class="btn btn-success edit_trans" target="_blank" href="javascript: void(0)" ><i class="fa fa-pencil"></i></a>
												<a title="Delete" data-id="<?php echo $pay_trans->tra_id; ?>" class="btn btn-danger del_trans" target="_blank" href="javascript: void(0)"><i class="fa fa-trash-o"></i></a>
											</td>
										</tr>
										<?php 
							...
							<div class="customer-details">	
								<div class=" ">
									...
									<div class="col-md-6 col-lg-6">
										...
										<div class="col-md-6 form_vr"><?php echo number_format($pay->advance_recieved) . " /-"; ?></div>
									</div>
									<div class="col-md-6 col-lg-6">
										<div class="col-md-6 form_vl"><strong>Advance Received Date:</strong></div>	
										<div class="col-md-6 form_vr"><?php echo display_month_name( $pay->booking_date ); ?></div>
									</div>
									...
									<div class="col-md-6 col-lg-6">
										<div class="col-md-6 form_vl"><strong>Second Installment Amount: (<?php echo $pd; ?>) </strong></div>	
									...
									<div class="col-md-6 col-lg-6">
										...
										<div class="col-md-6 form_vr"><?php echo display_month_name( $pay->second_payment_date ); ?></div>
									</div>
									<?php } ?>
									...
									<div class="col-md-6 col-lg-6">
										<div class="col-md-6 form_vl"><strong>Third Installment Amount: (<?php echo $pd; ?>)</strong></div>	
									...
									<div class="col-md-6 col-lg-6">
										...
										<div class="col-md-6 form_vr"><?php echo display_month_name ( $pay->third_payment_date ); ?></div>
									</div>
									<?php } ?>
									...
									<div class="col-md-6 col-lg-6">
										<div class="col-md-6 form_vl"><strong>Final Installment Amount: (<?php echo $pd; ?>)</strong></div>	 
									...
									<div class="col-md-6 col-lg-6">
										...
										<div class="col-md-6 form_vr"><?php echo display_month_name( $pay->final_payment_date ); ?></div>
									</div>
									<?php } ?>
									<?php if( !empty( $pay->total_discount ) ){ ?>
										<div class="col-md-6 col-lg-6">
											<div class="col-md-6 form_vl"><strong class='red'>Total Discount:</strong></div>	 
											<div class="col-md-6 form_vr"><?php echo number_format($pay->total_discount) . " /-"; ?></div>
										</div>
										<div class="col-md-6 col-lg-6">
									...
									<?php if( $refund_transaction ){ ?>
										<div class="col-md-6 col-lg-6">
											...
											<div class="col-md-6 form_vr"><strong class='red'><?php echo number_format($r_amount) . " /-"; ?></strong></div>
										</div>
									<?php } ?>
							...
							<div class="tab-content ">
								<div class="portlet-body tabbable-line">
									...
									<div class="tab-content">
										...
										<div class="tab-pane fade active in" id="pvtab_1_1">
											<form id="update_payment">
												<h3 class="text-center">RECIEVE PAYMENT</h3>
												<div class="form-group col-md-3">
													<label for="usr">Total Package Cost:</label>
													<input type="text"  readonly class="form-control"  value="<?php echo $pay->total_package_cost; ?>" />
												</div>
												
												<div class="form-group col-md-3">
													<label for="usr">Total Amount Received:</label>
													<input type="text"  readonly class="form-control" value="<?php echo $total_amount_received; ?>" />
												</div>
												...
												<div class="form-group col-md-3">
													...
													<input type="text" id="invoice_date" required name="invoice_date" placeholder="Invoice Date" class="form-control invoice_date" value="" >
												</div>
												<div class="clearfix"></div>
												
												<div class="form-group col-md-4">
													<label class=""><strong>Inst. Amount To Receive:</strong></label>&nbsp; <strong class="red"><?php echo number_format( $nextPay ); ?> /- </strong> Ins Date : <?php echo $pay->next_payment_due_date; ?>
												...
												<?php if( $inst_count < 3 ){ ?>
													<div class="form-group col-md-4">
														<label class=""><input type="checkbox" class="extraCheck" value="Yes"> <strong>Adjustment In Amount:</strong></label>
													</div>
												<?php }  ?>
												
												<!--DISCOUNT BUTTON -->
												<div class="form-group col-md-4">
													<label class=""><input type="checkbox" class="discountCheck" value="Yes"> <strong>Discount:</strong></label>
													<input type="number" readonly id="discount" name="discount" placeholder="Discount Price" class="form-control" value="">
												</div>
													...
													case 1: ?>
														...
														<div class="nextInstallments">
															...
															<div class="form-group col-md-4">
																...
																<input type="number" id="third_pay_balance" name="third_payment_bal" placeholder="Third Installment Amount" class="form-control pending_pay" data-pre_amount="<?php echo $pay->third_payment_bal; ?>" value="<?php echo $pay->third_payment_bal; ?>">
															</div>
															<div class="form-group col-md-4">
															...
															<div class="form-group col-md-4">
																...
																<input type="number" readonly="readonly"  id="final_pay_balance" name="final_payment_bal" placeholder="Final Installment Amount" class="form-control pending_pay" data-pre_amount="<?php echo $pay->final_payment_bal; ?>" value="<?php echo $pay->final_payment_bal; ?>">
															</div>
															<div class="form-group col-md-4">
																<label class=""><strong>Final Installment Due Date:</strong></label>
																<input readonly="readonly" data-date-format="yyyy-mm-dd" class="input-group form-control date_picker" id="final_payment_date" data-pre_date ="<?php echo $pay->final_payment_date; ?>" type="text" value="<?php echo $pay->final_payment_date; ?>" name="final_payment_date"  />
															</div>
														</div>
													<?php break;
													case 2: ?>
												...
												<div class="form-group col-md-3">
													<div class="form-group2">
														...
														<input readonly="readonly" required data-date-format="yyyy-mm-dd" class="input-group form-control date_picker" type="text" value="<?php echo $pay->travel_date; ?>" name="travel_date"  />
													</div>	 
												</div> */ ?>
												
												<div class="form-group col-md-3">
													<label class=""><strong>Balance Due After Payment Receive:</strong></label>
													<input type="number" readonly id="due_balance" name="new_due_balance" data-pre_balance="<?php echo $newBalPending; ?>" placeholder="Due Balance" class="form-control" value="<?php echo $newBalPending; ?>">
												</div>
												<div class="clearfix"></div>
											...
											<!--show if third payment is pending-->
												...
												<?php if( !empty($pay->second_payment_bal) && ( $pay->second_pay_status == "unpaid") ) { ?>
													<div class="form-group col-md-4">
														...
														<input type="number" readonly class="form-control" value="<?php echo $pay->second_payment_bal; ?>">
													</div>
													<div class="form-group col-md-4">
														<label class=""><strong>Second Installment Due Date:</strong></label>
														<input readonly="readonly" required data-date-format="yyyy-mm-dd" class="input-group form-control postpone_date" id="second_payment_date_post" type="text" data-pre_date ="<?php echo $pay->second_payment_date; ?>" value="<?php echo $pay->second_payment_date; ?>" name="second_payment_date"  />
													</div>
													<div class="clearfix"></div>
												<?php } ?>
												<!--show if third payment is pending-->
												<?php if( !empty($pay->third_payment_bal) && ( $pay->third_pay_status == "unpaid") ){ ?>
													<div class="form-group col-md-4">
														<label class=""><strong>Third Installment Amount:</strong></label>
														<input type="number" readonly  class="form-control" value="<?php echo $pay->third_payment_bal; ?>">
													</div>
													<div class="form-group col-md-4">
														<label class=""><strong>Third Installment Due Date:</strong></label>
														<input readonly="readonly" required data-date-format="yyyy-mm-dd" class="input-group form-control postpone_date" id="third_payment_date_post" type="text" data-pre_date ="<?php echo $pay->third_payment_date; ?>" value="<?php echo $pay->third_payment_date; ?>" name="third_payment_date"  />
													</div>
													<div class="clearfix"></div>
												<?php } ?>
												...
												<?php if( !empty($pay->final_payment_bal) && ( $pay->final_pay_status == "unpaid") ){ ?>
													<div class="form-group col-md-4">
														<label class=""><strong>Final Installment Amount:</strong></label>
														<input type="number" readonly="readonly" class="form-control pending_pay" value="<?php echo $pay->final_payment_bal; ?>">
													</div>
													<div class="form-group col-md-4">
														<label class=""><strong>Final Installment Due Date:</strong></label>
														<input readonly="readonly" data-date-format="yyyy-mm-dd" required class="input-group form-control postpone_date" id="final_payment_date_post" data-pre_date ="<?php echo $pay->final_payment_date; ?>" type="text" value="<?php echo $pay->final_payment_date; ?>" name="final_payment_date"  />
													</div>
												<?php } ?>
												...
												<input type="hidden" name="ins_number" value="<?php echo $inst_count; ?>">
												<div class="margiv-top-10">
													<button type="submit" class="btn green uppercase submit_pfrm" id="submit_pfrm">Submit</button>
												</div>
												<div class="clearfix"></div>
									...
									<?php if( !empty( $pay->amendment_note ) ){ ?>
										<div class='clearfix well well-sm text-center'><strong>Amendment Note : </strong> <span class='red'><?php echo $pay->amendment_note; ?></span></div>
									<?php } ?>	
								</div>	
							</div>	
						</div>	
					</div>
				</div><!--eND IF Payment Pending-->				
				<?php }	?>
			</div>
			<?php }else{
				redirect("payments");
...
<!-- Booking Payment Script -->
 <style>#successModal, #closeItiModal, #editPayModal{top: 20%;}</style>
<!-- Modal -->
				...
				<form id="frm_update_closeIti">
					...
					<div class="frm_section">
						...
						<div class="col-md-6">													
							...
							<input required type="number" readonly value="<?php echo $pay->total_package_cost; ?>" class="form-control" id="pkup">
						</div>						
						<div class="col-md-6">
							<div class="form-group">
							  <label for=""><strong>Balance Pending:</strong></label>
							  <input required type="number" readonly value="<?php echo $pay->total_balance_amount; ?>" class="form-control">
							</div>
						</div>
						<div class="clearfix"></div>
						<div class="col-md-6">
							<div class="form-group">
							  <label for="pkup">Add New Final Cost Of Package: (<?php echo "<strong class='red'>Min: " . $min_new_cost . " - Max : " . $pay->total_package_cost . " </strong>"; ?>)</label>
							  <input required name="total_package_cost" type="number" min="<?php echo $min_new_cost; ?>" max="<?php echo $pay->total_package_cost; ?>" value="" class="form-control" id="newfinalcost">
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
							  <label for=""><strong>Customer Name*:</strong></label>
							  <input required name="customer_name" type="text" value="<?php echo $pay->customer_name; ?>" class="form-control">
							</div>
						</div>
						<div class="clearfix"></div>
						<hr>
			...
			<div class="modal-body text-center"> 
				<form id="frm_edit_tra_detail">
					...
					<div class="form-group col-md-6">
						<label class=""><strong>Invoice Date*:</strong></label>
						<input type="text" id="einvoice_date" required name="einvoice_date" placeholder="Invoice Date" class="form-control einvoice_date" value="" >
					</div>
					<div class="clearfix"></div>
					
					<div class="col-md-6">													
						<label class=" "><strong>Bank Name*:</strong></label>
						<input required type="text" value="" name="tra_bank_name" class="form-control" id="e_t_bank_name">
					</div>
					
					<div class="col-md-6">													
						<label class=" "><strong>Payment Type*:</strong></label>
						<input required type="text" value="" name="e_t_payment_type" class="form-control" placeholder="online/offline etc."  id="e_t_payment_type">
					</div>
					<div class="clearfix"></div>
					<hr>
...
jQuery(document).ready(function($){
	...
	$(document).on("blur", "#current_payment", function(){
		...
		_this_val > 1 && (due_balance <= 0 ) ? $(".nextInstallments").hide() : $(".nextInstallments").show();
		
		if( _this_val > total_balace){
			$(".resPonse").html('<div class="alert alert-danger"><strong>Error! </strong>Extra Payment should be less than or equal Balance amount '+  total_balace +'</div>');
			...
			var THIRDINST = INS_NUMBER == 1 && FIN_INPUT == 0 ? $("#third_pay_balance").val(due_balance) : "";
		}
		
		//remove attr readonly if balance is zero if having third and final installment pending
		_this_val > 1 && total_pending_ins > 1 ? $("#third_pay_balance").removeAttr("readonly") : $("#third_pay_balance").attr("readonly", "readonly");
		
		_this_val > 1 && total_pending_ins > 1 ? $("#third_pay_balance, #third_payment_date").attr("required", "required") : $("#third_pay_balance,#third_payment_date").removeAttr("required");
	...
	$(document).on("blur", "#third_pay_balance", function(){
		...
		var addThisRequired = pendingBal >= 1 ? $("#third_pay_balance,#third_payment_date").attr("required", "required") : $("#third_pay_balance,#third_payment_date").removeAttr("required");
		...
		if( _this_val > pendingBal ){
			$(".resPonse").html('<div class="alert alert-danger"><strong>Error! </strong>Next Payment should be less than Pending amount = '+ pendingBal +'</div>');
			_this.val("");
	...
	$(document).on("blur", "#discount", function(){
		...
		if( _this_val > currentIns && validate){
			$(".resPonse").html('<div class="alert alert-danger"><strong>Error! </strong>Discount should be less than or equal Balance amount '+  currentIns +'</div>');
			$("#discount").val("");
	...
	$("#frm_update_closeIti").validate({
		submitHandler: function(form, event) {
			...
			if ( confirm("Are you sure to proceed ? Please make sure that the amount is correct. : You are enter RS. " + TOTALNEWCOST + " /-") ){
				updatecloseiti();
			}	
