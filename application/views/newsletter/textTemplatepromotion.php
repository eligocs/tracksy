		...
		<div class="container">
			<div class="page-content">
				...
				<?php 
						...
						if($news_uri == "templateImage"){ 
						if($image){ 	$temp = $image[0]; 
						?>
					<div class="newsletter_section image">	

						<div class=" text-center">
					  <img width="100%" src="<?php echo base_url('site/images/imageTemplate/').$temp->img_name; ?> "> 
					</div>
				</div>
				<?php }} elseif($news_uri == "templateText"){ ?>
				<?php $text = $template[0];
				if($text){ ?>
				<div class="newsletter_section promotion">
				<h4 class='greeting-text'>
					Dear <?php echo htmlspecialchars_decode($text->greeting); ?>
				</h4>
				
				<div class="offer pull-right">
					<div class="offer-text">Offer <span><?php echo htmlspecialchars_decode($text->offer); ?></span></div>
				...
				<div class="offer2">
					<div class="offer-text2"><span class="text-offer">Offer</span> <span class="title-offer"><?php echo htmlspecialchars_decode($text->offer); ?></span></div>
				</div>
				
				
				
					<div class="iti_list">
							<h2 class="iti_heading">Day Wise Itinerary</h2>
					<?php  $day = unserialize($text->day_wis_Iti); 
							$count_meta = count( $day );
							//dump($day);die;
							if( !empty($day) ){
								$r=1; for($i = 0; $i < $count_meta; $i++ ) { 
								echo "<div class='day_wise_iti'><span class='day' >Day<span class='day_no'>".$r++."</span></span>".$day[$i]['iti']."</div>"; 
							}}?>
							...
							<h3>Inclusion</h3>
							<ul>
							<?php  $inclusion = unserialize($text->inclusion); 
							$inclu = count( $inclusion );
							//dump($day);die;
							if( !empty($inclusion) ){
								
								for($i = 0; $i < $inclu; $i++ ) { 
								echo "<li><i class='fa fa-check'></i>".$inclusion[$i]['inclusion'].'</li>';
				...
				<div class="hotel-p-t text-center">		
					<div class="col-md-6">
					<h4><i class="fa fa-calendar"></i> Start From</h4>	
					<p><?php echo $text->hotel_price; ?></p>
					</div>
					<div class="col-md-6">
					<h4><i class="fa fa-hotel"></i> Hotel Type</h4>	
					<p><?php echo $text->hotel_type; ?></p>
				<?php } ?>
					</div>		
